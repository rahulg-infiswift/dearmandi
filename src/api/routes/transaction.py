...
async def create_transaction(
    ...
    commodity = await commodity_collection.find_one({"_id": ObjectId(transaction.commodity_id)})
    if not commodity:
    ...
    counterparty = await counterparty_collection.find_one({"_id": ObjectId(transaction.counterparty_id)})
    if not counterparty:
...
async def update_transaction(
    ...
    if 'commodity_id' in update_data:
        if not ObjectId.is_valid(update_data['commodity_id']):
            raise HTTPException(status_code=400, detail="Invalid commodity ID")
        commodity = await commodity_collection.find_one({"_id": ObjectId(update_data['commodity_id'])})
        if not commodity:
            raise HTTPException(status_code=404, detail="Commodity not found")
    ...
    if 'counterparty_id' in update_data:
        if not ObjectId.is_valid(update_data['counterparty_id']):
            raise HTTPException(status_code=400, detail="Invalid counterparty ID")
        counterparty = await counterparty_collection.find_one({"_id": ObjectId(update_data['counterparty_id'])})
        if not counterparty:
            raise HTTPException(status_code=404, detail="Counterparty not found")
